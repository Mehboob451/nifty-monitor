...
from . import alerts, config, engine, gemini, indicators, market_data, news, options, scoreboard
...
def run(now, src, force=False):
    ...
    ind, tparts, tscore = indicators.compute(m5, daily, data_date)
    ...
    scores = {"technical": tscore, "options": oscore, "global": gscore, "news": nres["score"]}
    calib = scoreboard.calibration(history)
    preds = engine.build_prediction(anchor, price, scores, m5, vix, calib)
    ...
    parts = {"technical": tparts, "options": oparts}
    reasoning = engine.rule_reasoning(scores, parts, preds, ind, an, glob, nres)
